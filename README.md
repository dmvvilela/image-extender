...
Powered by Google Gemini and OpenAI image models via the [Vercel AI SDK](https://ai-sdk.dev),
...
- **BYOK (Bring Your Own Key).** Google and OpenAI keys are stored only in your
  browser's `localStorage`. The server proxies your requests to the provider
  but never logs or persists the keys.
- **Model picker.** Switch between Gemini 3 Pro Image (Nano Banana Pro),
  Gemini 3 Flash Image (Nano Banana 2), Gemini 2.5 Flash Image (Nano
  Banana), and GPT-5.4 Image 2 from Settings — each with tuned best-of-N and timing.
...
will prompt for a Google AI API key (default model) — paste it once, it's stored locally,
you'll never see the prompt again unless you clear it from Settings.

Get a Google key at [aistudio.google.com/apikey](https://aistudio.google.com/apikey).
For OpenAI models, add an OpenAI key from [platform.openai.com/api-keys](https://platform.openai.com/api-keys) in Settings.
...
# edit .env.local and add GOOGLE_GENERATIVE_AI_API_KEY and/or OPENAI_API_KEY
```
...
- **[Vercel AI SDK](https://ai-sdk.dev)** (`ai`, `@ai-sdk/google`, `@ai-sdk/openai`) for direct provider access
...
app/
├── api/
│   ├── extend/route.ts        Outpainting endpoint (AI SDK → Google/OpenAI)
...
- API keys entered in the UI are stored **only** in your
  browser's `localStorage`. They are never written to the server's disk and
  never logged. The server uses them once per request to call Google or OpenAI,
  then discards them.
- The server-side `GOOGLE_GENERATIVE_AI_API_KEY` and `OPENAI_API_KEY` env vars are **optional** and act only
  as fallbacks for requests that don't include client-provided keys.
...
- Google for the Gemini image models, OpenAI for GPT image models, Vercel AI SDK for the provider layer.
